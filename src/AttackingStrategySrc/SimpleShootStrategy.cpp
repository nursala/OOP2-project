...
bool SimpleShootStrategy::attack(Character& self) {
	...
	auto bullets = weapon->fire(self.getWorld(), bulletPosB2, direction, self.shared_from_this());
	...
	SoundManager::instance().play(Constants::WeaponDataMap.at(weapon->getType()).shootSound);
	SoundManager::instance().setVolume(Constants::WeaponDataMap.at(weapon->getType()).shootSound, 80.f);
	// Fire the weapon and pass the bullet(s) to the world.
	self.getWorld().addBullets(std::move(bullets));
